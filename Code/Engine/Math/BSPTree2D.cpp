#include "Engine/Math/BSPTree2D.hpp"
#include "Engine/Math/ConvexShape2D.hpp"
#include "Engine/Math/MathUtils.hpp"
#include <algorithm>
#include <cmath>

void BSPTree2D::Build(Vec2 const& worldMins, Vec2 const& worldMaxs,
...
void BSPTree2D::BuildRecursive(int nodeIndex, std::vector<ConvexShape2D> const& shapes,
	int depth, int maxDepth, int maxLeafSize)
{
	BSPNode2D& node = m_nodes[nodeIndex];
...
void BSPTree2D::RaycastCandidates(Vec2 const& start, Vec2 const& fwdNormal, float maxDist,
...
void BSPTree2D::RaycastNode(int nodeIndex, Vec2 const& start, Vec2 const& fwdNormal,
	float tMin, float tMax, std::vector<bool>& visited, std::vector<int>& outCandidates) const
{
	...
	BSPNode2D const& node = m_nodes[nodeIndex];
